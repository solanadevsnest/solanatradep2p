...
use crate::utils::{
    create_account, initialize_token_account, transfer_native_to_account, transfer_token_to_account,
};
...
pub fn initiate_trade_setup<'info>(
    ...
    let trade_type: TradeType = match creator_accounts {
        ...
        (Ok(creator_send_account), Ok(creator_receive_account)) => match mints_address {
            (Some(Ok(mint_token_creator_trade)), Some(Ok(mint_token_creator_receive))) => {
                ...
                ctx.accounts.transfer_token_to_vault(params.deal_amount)?;

                msg!("Created trading P2P between SPL <-> SPL. Now ready for trade");

                //
                ctx.accounts.escrow_state.creator_send_token_mint =
                    Some(mint_token_creator_trade.key());
                ctx.accounts.escrow_state.creator_receive_token_mint =
                    Some(mint_token_creator_receive.key());
                //
        ...
        (Ok(creator_send_account), Err(_)) => match mints_address {
        ...
        (Err(_), Ok(creator_receive_account)) => match mints_address {
            (Some(Ok(mint_token_creator_receive)), None) => {
                ...
                ctx.accounts
                    .create_native_account_vault(vault_bump, params.order_id)?;
                // transfer SOL -> Vault Escrow
                ctx.accounts.transfer_native_vault(params.deal_amount)?;
    ...
    ctx.accounts.escrow_state.deal_amount = params.deal_amount;
